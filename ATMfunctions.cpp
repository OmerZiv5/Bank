...
void create_account(int account_number,int password ,int initial_amount, int ATMid){
    ...
    if(it == bank.accounts.end()){
        ...
        it = bank.accounts.end() - 1;
        pthread_mutex_lock(&write_log);
        cout << ATMid << ": New account id is " << new_acc.account_num << " with password " << new_acc.password << " and initial balance " << new_acc.balance << endl;
        pthread_mutex_unlock(&write_log);
    }
    else{
        // Account already exists
        pthread_mutex_lock(&write_log);
        cout << "Error " << ATMid << ": Your transaction failed - account with the same id exists" << endl;
        pthread_mutex_unlock(&write_log);
    }
...
void deposit_to_account(int account_number,int password ,int amount, int ATMid){
    ...
    if(it == bank.accounts.end()){
        pthread_mutex_lock(&write_log);
        cout << "Error " << ATMid << ": Your transaction failed - account id " << account_number << " does not exist" << endl;
        pthread_mutex_unlock(&write_log);
    }
    else{
        ...
        if(it->password != password){
            ...
            pthread_mutex_lock(&write_log);
            cout << "Error " << ATMid << ": Your transaction failed - password for account id " << account_number << " is incorrect" << endl;
            pthread_mutex_unlock(&write_log);
        }
        else{
            // Password is correct
            it->balance += amount;
            pthread_mutex_lock(&write_log);
            cout << ATMid << ": Account " << it->account_num << " new balance is " << it->balance << " after amount " << amount << " $ was deposited" << endl;
            pthread_mutex_unlock(&write_log);
        }
...
void withdraw_from_account(int account_number,int password ,int amount, int ATMid){
    ...
    if(it == bank.accounts.end()){
        ...
        if(it->password != password){
            ...
            pthread_mutex_lock(&write_log);
            cout << "Error " << ATMid << ": Your transaction failed - account id " << account_number << " balance is lower than" << amount << endl;
            pthread_mutex_unlock(&write_log);
        }
        else{
            // withdraw successful
            it->balance -= amount;
            pthread_mutex_lock(&write_log);
            cout << ATMid << ": Account " << it->account_num << " new balance is " << it->balance << " after " << amount << " $ was withdrew" << endl;
            pthread_mutex_unlock(&write_log);
        }
        pthread_mutex_unlock(&it->write_account);
    }
...
void check_balance(int account_number, int password, int ATMid){
    ...
    if(it == bank.accounts.end()){
        ...
        if(it->password != password){
            ...
            // password is correct
            pthread_mutex_lock(&write_log);
            cout << ATMid << ": Account " << account_number << " balance is " << it->balance << endl;
            pthread_mutex_unlock(&write_log);
        }
...
void close_account(int account_number, int password, int ATMid){
    ...
    if(it == bank.accounts.end()){
        ...
        if(it->password != password){
            ...
            pthread_mutex_lock(&write_log);
            cout << ATMid << ": Account " << account_number << " is now closed. Balance was " << balance << endl;
            pthread_mutex_unlock(&write_log);
        }
    }
...
void transfer(int source_account, int source_password, int target_account, int amount, int ATMid){
    ...
    if(source_it == bank.accounts.end()){
        pthread_mutex_lock(&write_log);
        cout << "Error " << ATMid << ": Your transaction failed - account id " << source_account << " does not exist" << endl;
        pthread_mutex_unlock(&write_log);
    }
    else if(source_it == bank.accounts.end()){
        pthread_mutex_lock(&write_log);
        cout << "Error " << ATMid << ": Your transaction failed - account id " << target_account << " does not exist" << endl;
        pthread_mutex_unlock(&write_log);
    }
    else{
        // both accounts exist
        ...
        if(source_it->password != source_password){
            pthread_mutex_lock(&write_log);
            cout << "Error " << ATMid << ": Your transaction failed - password for account id " << source_account << " is incorrect" << endl;
            pthread_mutex_unlock(&write_log);
        }
        else if(source_it->balance < amount){
            pthread_mutex_lock(&write_log);
            cout << "Error " << ATMid << ": Your transaction failed - account id " << source_account << " balance is lower than" << amount << endl;
            pthread_mutex_unlock(&write_log);
        }
        else{
